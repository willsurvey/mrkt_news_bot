...
// FIX: Custom fetch with timeout (tanpa AbortController conflict)
...
export async function fetchRSS(source, maxRetries = 2) {
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      // FIX: Use fetchWithTimeout + parseString instead of parseURL
      // This avoids AbortController conflict with rss-parser internal callbacks
      const xmlContent = await fetchWithTimeout(trimmedUrl, timeout);
      
      const feed = await parser.parseString(xmlContent);
      
      // FIX: Validate feed structure comprehensively
      if (!feed) {
        throw new Error('Feed is null or undefined');
      }
      
      if (!feed.items) {
        throw new Error('Feed has no items property');
      }
      
      if (!Array.isArray(feed.items)) {
        // Some feeds return items as object instead of array
        if (typeof feed.items === 'object') {
          feed.items = [feed.items];
        } else {
          throw new Error(`Feed items is not an array (type: ${typeof feed.items})`);
        }
      }
