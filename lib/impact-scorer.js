...
export function calculateTopicScore(article) {
  ...
  for (const keyword of TOPIC_SCORES.L1_MACRO.keywords) {
    if (text.includes(keyword.toLowerCase())) return TOPIC_SCORES.L1_MACRO.score;
  }
  for (const keyword of TOPIC_SCORES.L2_MARKET.keywords) {
    if (text.includes(keyword.toLowerCase())) return TOPIC_SCORES.L2_MARKET.score;
  }
  for (const keyword of TOPIC_SCORES.L3_SECTOR.keywords) {
    if (text.includes(keyword.toLowerCase())) return TOPIC_SCORES.L3_SECTOR.score;
  }
  for (const keyword of TOPIC_SCORES.L4_ISSUER.keywords) {
    if (text.includes(keyword.toLowerCase())) return TOPIC_SCORES.L4_ISSUER.score;
  }
...
export function calculateKeywordMultiplier(article) {
  ...
  for (const keyword of KEYWORDS.HARD) {
    if (text.includes(keyword.toLowerCase())) hardCount++;
  }
  for (const keyword of KEYWORDS.SOFT) {
    if (text.includes(keyword.toLowerCase())) softCount++;
  }

  if (hardCount >= 2) return 1.5;
  if (hardCount === 1) return 1.3;
  if (softCount > hardCount) return 0.8;
  return 1.0;
}
...
export function calculateScopeMultiplier(article) {
  ...
  if (
    text.includes('ihsg') ||
    text.includes('pasar saham indonesia') ||
    text.includes('indeks utama') ||
    text.includes('market-wide')
  ) {
    return 1.5;
  }

  if (text.includes('multi sektor') || text.includes('beberapa sektor')) return 1.2;
  if (text.includes('sektor') || text.includes('industri')) return 1.0;

  // Emiten individual (kode saham 4 huruf)
  if ((article.title || '').match(/\b[A-Z]{4}\b/)) return 0.8;
...
export function classifyImpact(score) {
  ...
  if (score >= IMPACT_THRESHOLDS.MED) return 'MED';
  return 'LOW';
}

export function hasExtremeKeyword(article) {
  const text = `${article.title || ''} ${article.description || ''}`.toLowerCase();
  return KEYWORDS.EXTREME.some(kw => text.includes(kw.toLowerCase()));
}
...
export function scoreArticle(article) {
  ...
  // Override: CORE sumber + L1_MACRO topik → minimum MED
  if (article.source_tier === 'CORE' && topicScore >= 90 && impact === 'LOW') {
    impact = 'MED';
  }
